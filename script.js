/*
  ...
  art & code by 
  Anna Zenn Scavenger, March 2021
...
let doodleLines = document.querySelectorAll('.doodle-lines');
let airbrushGroup = document.querySelectorAll('.airbrush');
...
function Sketch() {
  ...
  function initSprinkles() {
    ...
    for (let i = 0; i < 70; i++) {
      ...
      let x =  (radius * Math.sin(phi) * Math.cos(theta)) + getRandomInRange(-10, 45);
  ...
  function updateAirbrushPositions() {
    
    let cx1 = Math.floor(getRandomInRange(-355, -200));
    let cy1 = Math.floor(getRandomInRange(80, 170));
    let r = Math.floor(getRandomInRange(10, 60));
    let r2 = Math.floor(getRandomInRange(30, 80));
    airbrush1.setAttribute('cx', `${cx1}`);
    airbrush1.setAttribute('cy', `${cy1}`);
    airbrush1.setAttribute('r', `${r}`);
    let cx2 = Math.floor(getRandomInRange(-420, -250));
    let cy2 = Math.floor(getRandomInRange(120, 170));
    airbrush2.setAttribute('cx', `${cx2}`);
    airbrush2.setAttribute('cy', `${cy2}`);
    airbrush3.setAttribute('r', `${r2}`);
    
  }
  
  function updateSVGLines() {
    
    doodleLines.forEach(doodle => doodle.style.strokeOpacity = "0");
    let length = doodleLines.length;
    let id = Math.floor(getRandomInRange(0, length));
    doodleLines[id].style.strokeOpacity = "0.9";
  ...
  function initSprinklesTimeline() {
    ...
    sprinklesTL
      .to({}, {x: 0, duration: 1, ease: "elastic" })
      .add("doodleEnd")
